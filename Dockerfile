...
# Acumulado M04 / calendário M06 exigem «_Planilha Modelo Kcor-Kria» em assets/templates/ (não só ponteiro LFS ~130 B).
RUN python -c "from pathlib import Path; p=Path('nc_artesp/assets/templates'); xs=[f for f in p.glob('_Planilha Modelo Kcor-Kria*') if f.is_file()]; mx=max((f.stat().st_size for f in xs), default=0); assert mx>2048, ('Kcor-Kria template em falta ou ponteiro LFS no build — ative Git LFS no Render e redeploy. Ficheiros: %r' % ([(f.name, f.stat().st_size) for f in xs],))"
